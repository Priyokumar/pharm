...
export class MedicineListComponent implements OnInit, AfterViewInit {
  ...
  getMedicines(){
    this.data = this.medicineService.getMedicines();
  ...
  addMedicine() {
    this.dialog.open(MedicineAddEditComponent, { width: "25%" }).afterClosed().subscribe(data => {
      if (data) {
        this.getMedicines();
      }
    });
  ...
  editMedicine(inventory: IMedicine) {
    ...
    this.dialog.open(MedicineAddEditComponent, { data: inventory, width: "35%" }).afterClosed().subscribe(
      data => {
        if (data) {
          this.getMedicines();
        }
  ...
  remove(data: IMedicine) {
    this.medicineService.removeMedicine(data.id);
