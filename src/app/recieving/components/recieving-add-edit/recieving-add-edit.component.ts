...
export class RecievingAddEditComponent implements OnInit, OnDestroy {
  ...
  processRecievingSubscription: Subscription;
  ...
  ngOnDestroy(): void {
    ...
    if (this.processRecievingSubscription)
      this.processRecievingSubscription.unsubscribe();
  }
      ...
      .subscribe((data: IRecievingItem) => {
        if (data) {
          const t = this.items.find((ele) => {
            return (
              ele.medicine.id === data.medicine.id &&
              ele.batch.batchNo === data.batch.batchNo
            );
          });
          if (t) {
  ...
  async save() {
    ...
    this.processRecievingSubscription = of(
      this.recievingService.addRecieving(payLoad)
    )
      .pipe(
        take(1),
        switchMap((data) => {
          return this.recievingToInventoryService.processRecieving(payLoad);
        })
      )
      .subscribe(
        (data) => {
